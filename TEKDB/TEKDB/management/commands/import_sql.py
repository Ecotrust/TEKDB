...
class Command(BaseCommand):
    help = (
        "Imports and rearranges SQL for PostgreSQL generated by Bullzip from Access DB"
    )
    ...
    def create_sql_dict(self, infile):
        ...
        for lookup in lookups:
            ...
            lookup_values[lookup] = {"NULL": "NULL"}
            # { 'Rocky Shore': 7 }
        ...
        lookup_dependencies = {
            ...
            "locality": {'"localitytype"': "lookuplocalitytype"},
            "media": {'"mediatype"': "lookupmediatype"},
            "places": {
                ...
                if 'INSERT INTO "' in line:
                    ...
                    if model in model_list:
                        ...
                        insert_dict[model]["inserts"].append(
                            {"table": model, "columns": columns, "values": values}
                        )
                    else:
                    ...
                    pattern = re.compile(
                        r'SELECT setval\(\'(?P<sequence>.*)\', MAX\((?P<column>.*)\)\) FROM "(?P<table>.*)";'
                    )
                    result = pattern.match(line)
                    model = result.groupdict()["table"].lower()
                    if model in model_list:
                        ...
                        insert_dict[model]["selects"].append(
                            {"sequence": sequence, "table": model, "column": column}
                        )
                    else:
                        print("%s - not found in Model List." % model)
                    ...
                    pattern = re.compile(
                        r'CREATE INDEX "(?P<index>.*)" ON "(?P<table>.*)"\s(?:USING\s.*\s)?\((?P<columns>.*)\);(?:\\n)?'
                    )
                    result = pattern.match(line)
                    if result:
                        model = result.groupdict()["table"].lower()
                        if model in model_list:
                            ...
                            insert_dict[model]["indices"].append(
                                {"index": index, "table": model, "columns": columns}
                            )
                        else:
                            print("%s - not found in Model List." % model)
                    # else:
    ...
    def delete_old_migrations(self, MANAGE_DIR):
        ...
        for module in self.get_app_list():
            ...
            migration_files = [
                x
                for x in migrations.iterdir()
                if not x.name == "__init__.py" and not x.is_dir()
            ]
    ...
    def import_sql_data(self, import_output, import_error, dbname, insert_script):
        ...
        call(
            ["psql", "-U", "postgres", "-d", dbname, "-f", insert_script],
            stdout=stdout,
            stderr=stderr,
        )
    ...
    def set_auto_pk_initial_values(
        self, import_output, import_error, dbname, update_script
    ):
        ############################################
        ...
        call(
            ["psql", "-U", "postgres", "-d", dbname, "-f", update_script],
            stdout=stdout,
            stderr=stderr,
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            print(
                "Enter c to contine, or type your desired dbname as `dbname = _____` hit enter, then c, then enter again."
            )
            import ipdb
        ...
        self.set_auto_pk_initial_values(
            import_output, import_error, dbname, update_script
        )
        ############################################
