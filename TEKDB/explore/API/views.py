...
class PageContentSingle(APIView):
    ...
    def get(self, request, name: str):
        try:
            ...
            page_content = (
                page_content_obj.html_content if page_content_obj.is_html else page_content_obj.content
            )
        except Exception:
...
class ExploreSearch(APIView):
        ...
        for category in categories:
            ...
            for model in query_models:
                ...
                for result in model_results:
                    ...
                    actual_attribute = self.remove_match_prefix(greatest_attr) if greatest_attr else None
                    verbose_name = (
                        self.get_verbose_field_name(model, actual_attribute) if actual_attribute else None
                    )
                    headline_key = f"headline_{actual_attribute}" if actual_attribute else None
                    headline_value = getattr(result, headline_key) if headline_key and hasattr(result, headline_key) else None
    ...
    def get(self, request):
        ...
        categories = request.GET.getlist("category") or [request.GET.get("category")] if request.GET.get("category") else []
        if categories == []:
    ...
    def get(self, request, model_type, id):
        ...
        if ("map" in record_dict and record_dict["map"] is not None) or any(
            rel.get("key") == "Place-Resource Events" and any(
                (val.get("map") is not None) for val in rel.get("value", [])
            )
                        ...
                        if ("key" in item and "value" in item and len(item.keys()) == 2):
                            if isinstance(item["value"], list) and len(item["value"]) > 0:
                                for sub_item in item["value"]:
                                    ...
                                    worksheet.write(row, 1, sub_item.get("name", str(sub_item)))
                                    row += 1
                            else:
                                worksheet.write(row, 0, f"{key} - {item['key']}")
                                try:
                            ...
                            if isinstance(item["value"], list) and len(item["value"]) > 0:
                                for sub_item in item["value"]:
                                    if isinstance(sub_item, dict) and "name" in sub_item:
                                        writer.writerow([f"{key} - {item['key']}", sub_item["name"]])
                                    else:
                                        writer.writerow([f"{key} - {item['key']}", str(sub_item)])
                            else:
                                writer.writerow([f"{key} - {item['key']}", item["value"]])
                        else:
                            for list_key in item.keys():
