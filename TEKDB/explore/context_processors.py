def explore_context(request):
        ...
        for key in proj_icons.keys():
            try:
                if hasattr(configs, key) and getattr(configs, key):
                    ...
                    if icon_value == "Other":
                        if hasattr(configs, "{}_override".format(key)) and getattr(
                            configs, "{}_override".format(key)
                        ):
                            abs_project_icon_override_filename = getattr(
                                configs, "{}_override".format(key)
                            ).file.name
                            rel_filename = abs_project_icon_override_filename.split(
                                settings.MEDIA_ROOT
                            )[-1]
                            icon_override_select = "{}{}".format(
                                settings.MEDIA_URL, rel_filename
                            )
    ...
    if configs:
        try:
            if hasattr(configs, "homepage_image") and getattr(
                configs, "homepage_image"
            ):
                abs_project_image_filename = getattr(
                    configs, "homepage_image"
                ).file.name
        ...
        try:
            if hasattr(configs, "homepage_image_attribution") and getattr(
    ...
    if configs:
        try:
            if hasattr(configs, "homepage_font_color") and getattr(
